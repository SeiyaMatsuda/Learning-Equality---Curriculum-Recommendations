...
def ensemble_postprocess_2(df, threshold:float = 0.8, weight:np.array = np.array([0.45, 0.2, 0.35]), top_n: int = 5):
    ...
    df['predictions_proba'] = df['predictions_proba_svm'] * weight[0] + df['predictions_proba_roberta_base'] * weight[1] + df['predictions_proba_roberta_large'] * weight[2] + df['predictions_proba_xgb']*weight[3]
    out_df = df.copy()
    out_df.loc[:,'pred'] = 0
