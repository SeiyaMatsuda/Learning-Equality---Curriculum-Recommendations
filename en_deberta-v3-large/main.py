...
class Stage2model(pl.LightningModule):
    def __init__(self, model_name, learning_rate, num_train_steps, steps_per_epoch, target_df=None):
        super().__init__()
...
def train_fold(df,correlations_df,  tokenizer, fold):
    ...
    lightning_model = Stage2model(CFG.MODEL, CFG.LR, num_train_step, steps_per_epoch, target_df)
    lightning_model.load_state_dict(torch.load(Path(f'./fold_{fold}').glob('*.ckpt')[0])["state_dict"])
