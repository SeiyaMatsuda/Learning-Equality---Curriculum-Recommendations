...
class CFG1:
    ...
    STAGE2_MODEL_NAME_ROBERTA = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_ROBERTA = Path('../xlm-roberta-base-all-minilm/fold_0/epoch=5-step=528.ckpt')
    STAGE2_MODEL_NAME_DEBERTA = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_DEBERTA = Path('../en_deberta-v3-large/fold_0/epoch=4-step=120.ckpt')
...
class CFG2:
    ...
    STAGE2_MODEL_NAME_ROBERTA = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_ROBERTA = Path('../xlm-roberta-base-pharaprase/fold_0/epoch=6-step=679.ckpt')
    STAGE2_MODEL_NAME_DEBERTA = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_DEBERTA = Path('../en_deberta-v3-large/fold_0/epoch=4-step=120.ckpt')
...
class CFG3:
    ...
    STAGE2_MODEL_NAME_ROBERTA = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_ROBERTA = Path('../xlm-roberta-base-all-mpnet/fold_0/epoch=8-step=900.ckpt')
    STAGE2_MODEL_NAME_DEBERTA = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_DEBERTA = Path('../en_deberta-v3-large/fold_0/epoch=4-step=120.ckpt')
...
class Stage2model_ROBERTA(pl.LightningModule):
...
def inference_stage2_pytorch(CFG, df):
    ...
    tokenizer = AutoTokenizer.from_pretrained(CFG.STAGE2_MODEL_NAME_ROBERTA)
    model = Stage2model_ROBERTA(CFG.STAGE2_MODEL_NAME_ROBERTA) 
    state = torch.load(CFG.STAGE2_PRETRAINED_PATH_ROBERTA, map_location = torch.device('cpu'))
    model.load_state_dict(state['state_dict'])
    dataset = MyDataset(tokenizer, no_en_df, labels=False)
    ...
    no_en_preds = inference_fn(CFG, dataloader, model)
    preds[no_en_df.index] = no_en_preds
    del model, tokenizer, dataset, dataloader
    gc.collect()
    torch.cuda.empty_cache()
    en_df = df[df.language=='en']
    tokenizer = AutoTokenizer.from_pretrained(CFG.STAGE2_MODEL_NAME_DEBERTA)
    model = Stage2model_DEBERTA(CFG.STAGE2_MODEL_NAME_DEBERTA) 
    state = torch.load(CFG.STAGE2_PRETRAINED_PATH_DEBERTA, map_location = torch.device('cpu'))
    model.load_state_dict(state['state_dict'])
    dataset = MyDataset(tokenizer, en_df, labels=False)
    func = lambda x:collate_fn(x, tokenizer)
    dataloader = DataLoader(dataset, batch_size=CFG.BATCH_PER_GPU, 
                            collate_fn=func,
                            shuffle=False, num_workers=CFG.NUM_JOBS, drop_last=False)
    en_preds = inference_fn(CFG, dataloader, model)
    preds[en_df.index] = en_preds
    del model, tokenizer, dataset, dataloader
    gc.collect()
    return preds
...
def ensemble_postprocess(df, columns:list, threshold:list, top_n: int = 5):
    ...
    return result
def ensemble_postprocess_2(df, threshold:float = 0.8, weight:np.array = np.array([0.45, 0.2, 0.35]), top_n: int = 5):
    # df['predictions_proba'] = (df['predictions_proba_svm'] * df['predictions_proba_roberta'] * df['predictions_proba_roberta']) ** (1/3)
    weight = weight / sum(weight)
    df['predictions_proba'] = df['predictions_proba_svm'] * weight[0] + df['predictions_proba_pytorch'] * weight[1] +df['predictions_proba_xgb']*weight[2]
    out_df = df.copy()
    out_df.loc[:,'pred'] = 0
...
def inference_pipline(CFG, topics_df, content_df, correlations_df, sample_submission, prev_df=None):
    ...
    if (prev_df is None) or ('predictions_proba_pytorch' not in prev_df.columns): 
        preds = inference_stage2_pytorch(CFG, df)
        df['predictions_proba_pytorch'] = preds
    return df
...
def main():
    ...
    for model_idx, CFG in enumerate(CFG_LIST):
        ...
        if CFG_COMMON.TEST_PRED:
            ...
            result = ensemble_postprocess(df, columns= ['predictions_proba_svm', 'predictions_proba_pytorch', 'predictions_proba_xgb'], 
...
if __name__ == '__main__':
    # result = minimize(main, x0=np.array([0.89333333, 0.79259259, 0.67037037, 0.45, 0.2, 0.35]), method='Nelder-Mead')
    main()
