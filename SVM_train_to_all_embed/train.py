...
class CFG:
    ...
    MODEL_LIST = [{'model_name':"sentence-transformers/all-MiniLM-L6-v2",'path':Path('../dpr_all-MiniLM-L6-v2-finetuned/fold_0/epoch_8_step_2610.ckpt'), 'pattern':0},
                  {'model_name':"sentence-transformers/paraphrase-multilingual-mpnet-base-v2",'path':Path('../dpr_paraphrase-multilingual-mpnet-base-v2-finetuned/fold_0/epoch_1_step_434.ckpt'), 'pattern':0},
                  {'model_name':"sentence-transformers/all-mpnet-base-v2",'path':Path('../dpr_all-mpnet-base-v2-finetuned/fold_0/epoch_5-step_1302.ckpt'), 'pattern':0},
                  {'model_name':"xlm-roberta-base",'path':Path('../xlm-roberta-base-all-mpnet/fold_0/epoch=8-step=900.ckpt'), 'pattern':1}
                 ]
    ...
    BATCH_SIZE = 25000
    NUM_JOBS = 4
    THRESHOLD = [0.6, 0.65, 0.7, 0.8, 0.9]
...
def get_stage2_embeddings(model, tokenizer, data:pd.Series):
    data = list(data.fillna(''))
    gap = 1000
    outs = []
    # uniform dynamic padding
    ...
    for i in tqdm.tqdm(range(0, len(data), gap), desc='tokenization'):
        ...
        with torch.no_grad():
            inputs = batch_tokens['input_ids'].to(CFG.DEVICE)
            attention_mask = batch_tokens['attention_mask'].to(CFG.DEVICE)
            out = model(inputs, attention_mask).last_hidden_state.mean(1)
            outs.append(out.cpu().numpy())
            del batch_tokens, inputs, attention_mask
            gc.collect()
    return np.concatenate(outs)
...
def get_embedding_DPR(model, tokenizer, topics_df, content_df, save_path=None):
    ...
    gc.collect()
    return topics_embedding_cudf, contents_embedding_cudf

def get_embedding_stage2(model, tokenizer, topics_df, content_df, save_path=None):
    save_path.mkdir(parents=True, exist_ok=True)
    try:
        contents_embedding = np.load(save_path / 'contents_embedding.npy')
    except:
        contents_embedding = get_stage2_embeddings(model, tokenizer, content_df['title'])
        np.save(save_path / 'contents_embedding', contents_embedding)
    try:
        topics_embedding = np.load(save_path / 'topics_embedding.npy')
    except:
        topics_embedding = get_stage2_embeddings(model, tokenizer, topics_df['title'])
        np.save(save_path / 'topics_embedding', topics_embedding)
    del model, tokenizer
    gc.collect()
    torch.cuda.empty_cache()
...
def get_knn_predictions(topics_df, content_df, topics_embedding_cudf, contents_embedding_cudf):
...
def inference_stage1(topics_df, content_df, correlations_df, sample_submission):
    ...
    topics_embedding_list = []
    contents_embedding_list = []
    
    for model_cfg in CFG.MODEL_LIST:
        model_name = model_cfg["model_name"]
        weight_path = model_cfg["path"]
        pattern = model_cfg["pattern"]
        if pattern==0:
            tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=True)
            tokenizer.add_tokens(["<|=t_sep=|>"], special_tokens=True)
            model = Stage1model(tokenizer, model_name)
            model.load_state_dict(torch.load(weight_path)["state_dict"])
            model.eval()
            model.to(CFG.DEVICE)
            topics_embedding_cudf, contents_embedding_cudf = get_embedding_DPR(model, tokenizer, topics_df, content_df, Path(model_name.split("/")[-1]))
            ...
            model.eval()
            model.to(CFG.DEVICE)
            topics_embedding_cudf, contents_embedding_cudf = get_embedding_stage2(model, tokenizer, topics_df, content_df, Path('emb_' + model_name.split("/")[-1]))
        else:
            continue
        topics_embedding_cudf.columns = [f'{model_name}_{i}' for i in range(len(topics_embedding_cudf.columns))] 
        contents_embedding_cudf.columns = [f'{model_name}_{i}' for i in range(len(contents_embedding_cudf.columns))] 
        topics_embedding_list.append(topics_embedding_cudf)
        contents_embedding_list.append(contents_embedding_cudf)
        del topics_embedding_cudf, contents_embedding_cudf
        gc.collect()
        torch.cuda.empty_cache()
    topics_embedding_cudf = cudf.concat(topics_embedding_list, axis=1)
    contents_embedding_cudf = cudf.concat(contents_embedding_list, axis=1)
    knn_prd_idx = get_knn_predictions(topics_df, content_df, topics_embedding_cudf, contents_embedding_cudf)
...
def main():
    ...
    for fold in range(CFG.N_FOLDS):
        ...
        best_score = 0 
        best_threshold = 0
        for threhold in CFG.THRESHOLD:
            score = calc_f2_score_mean(target_df, postprocess(val_df, threshold=threhold, top_n = 5))
            if best_score<score:
                best_score = score
                best_threshold = threhold
        LOGGER.info(f"threshold:{best_threshold} score:{best_score}")
