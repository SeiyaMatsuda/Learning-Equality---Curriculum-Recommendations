...
class CFG:
    ...
    STAGE1_MODEL_PATH = './epoch_8_step_2610.ckpt'
    BATCH_PER_GPU = 128
    SEED=42
    NUM_EPOCHS=12
    LR = 1e-5
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs_t =  self.tokenizer.encode_plus(
        topic_text, 
        return_tensors = None, 
        ...
        topic_input_ids = inputs_t['input_ids']
        topic_attention_mask = inputs_t['attention_mask']
        content_input_ids = inputs_c['input_ids']
        content_attention_mask = inputs_c['attention_mask']
        if self.labels:
...
class Stage2model(pl.LightningModule):
    def __init__(self, tokenizer, model_name, learning_rate, num_train_steps, steps_per_epoch, target_df):
        super().__init__()
        ...
        self.output = nn.Conv1d(256, 1, kernel_size=2, padding=1)
        self.target_df = target_df
        
    def forward(self, ids1, mask1, ids2, mask2, targets):
        output_topics_encoder = self.topics_encoder(ids1, mask1)
        output_content_encoder = self.content_encoder(ids2, mask2)
        topics_embeddings = output_topics_encoder.last_hidden_state.permute(0, 2, 1)
        content_embeddings =  output_content_encoder.last_hidden_state.permute(0, 2, 1)
        sequence_output = torch.concat([topics_embeddings, content_embeddings], dim=2)
        sequence_output = F.relu(self.cnn1(sequence_output))
        logits1 = torch.max(self.output(self.dropout1(sequence_output)),2)[0]
        logits2 = torch.max(self.output(self.dropout2(sequence_output)),2)[0]
        logits3 = torch.max(self.output(self.dropout3(sequence_output)),2)[0]
        logits4 = torch.max(self.output(self.dropout4(sequence_output)),2)[0]
        logits5 = torch.max(self.output(self.dropout5(sequence_output)),2)[0]
    ...
    def training_step(self, batch, batch_idx):
        ids1, mask1, ids2, mask2, targets =  batch['topic_input_ids'], batch['topic_attention_mask'], batch['content_input_ids'], batch['content_attention_mask'], batch['labels']
        _, loss = self.forward(ids1=ids1, mask1=mask1, ids2=ids2, mask2=mask2, targets=targets)
        self.log("train_loss", loss, on_step=True, logger=True, prog_bar=True)
        return loss
    
    def validation_step(self, batch, batch_idx):
        ids1, mask1, ids2, mask2, targets = batch['topic_input_ids'], batch['topic_attention_mask'], batch['content_input_ids'], batch['content_attention_mask'], batch['labels']
        logits, loss = self.forward(ids1=ids1, mask1=mask1, ids2=ids2, mask2=mask2, targets=targets)
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        threholds = np.arange(0.05, 0.95, 0.1)
...
def train_fold(df,correlations_df,  tokenizer, fold):
    ...
    state = torch.load(CFG.STAGE1_MODEL_PATH, map_location = torch.device('cpu'))
