...
class CFG:
  ...
  PRETRAINED_PATH = Path('./epoch_5-step_1302.ckpt')
...
def get_knn_predictions(model, tokenizer, topics_df, content_df):
    ...
    gc.collect()
    return knn_prd_idx, topics_df, content_df

def build_trian_set(preds_df, topics_df, content_df, correlations_df):
    topics_id2title = {id:input_text for id, input_text in zip(topics_df.id, topics_df.title)}
    topics_id2lang = {id:lang for id, lang in zip(topics_df.id, topics_df.language)}
    content_id2title = {id:input_text for id, input_text in zip(content_df.id, content_df.title)}
    ...
    return pd.DataFrame({'topics_id':topic_id_list,'content_id':contents_id_list, 
        'topic_title':topic_title_list, 'content_title':contents_title_list, 'language':topic_lang_list, 'label':labels, 'fold':folds})
...
def main():
    ...
    knn_prd_idx, topics_df, content_df = get_knn_predictions(model, tokenizer, topics_df, content_df)
