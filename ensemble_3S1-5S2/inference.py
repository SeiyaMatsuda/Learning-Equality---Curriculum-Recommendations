...
class CFG1:
    ...
    STAGE2_MODEL_NAME_1 = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_1 = Path('../xlm-roberta-base-all-minilm/fold_0/epoch=5-step=528.ckpt')
    STAGE2_MODEL_NAME_2 = "xlm-roberta-large"
    STAGE2_PRETRAINED_PATH_2 = Path('../xlm-roberta-large/fold_0/epoch_6-step_616.ckpt')
    STAGE2_MODEL_NAME_3 = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_3 = Path('../en_deberta-v3-large/fold_0/epoch=10-step=308.ckpt')
...
class CFG2:
    ...
    STAGE2_MODEL_NAME_1 = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_1 = Path('../xlm-roberta-base-pharaprase/fold_0/epoch=6-step=679.ckpt')
    STAGE2_MODEL_NAME_2 = "xlm-roberta-large"
    STAGE2_PRETRAINED_PATH_2 = Path('../xlm-roberta-large/fold_0/epoch_6-step_616.ckpt')
    STAGE2_MODEL_NAME_3 = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_3 = Path('../en_deberta-v3-large/fold_0/epoch=10-step=308.ckpt')
...
class CFG3:
    ...
    STAGE2_MODEL_NAME_1 = "xlm-roberta-base"
    STAGE2_PRETRAINED_PATH_1 = Path('../xlm-roberta-base-all-mpnet/fold_0/epoch=8-step=900.ckpt')
    STAGE2_MODEL_NAME_2 = "xlm-roberta-large"
    STAGE2_PRETRAINED_PATH_2 = Path('../xlm-roberta-large/fold_0/epoch_6-step_616.ckpt')
    STAGE2_MODEL_NAME_3 = "microsoft/deberta-v3-large"
    STAGE2_PRETRAINED_PATH_3 = Path('../en_deberta-v3-large/fold_0/epoch=10-step=308.ckpt')
...
def build_trian_set(preds_df, topics_df, content_df, correlations_df, labels=False):
    ...
    topics_id2lang = {id:lang for id, lang in zip(topics_df.id, topics_df.language)}
    ...
    for idx, (c, t) in tqdm.tqdm(enumerate(zip(pred_contents, pred_topic)), total=len(pred_contents)):
        ...
        topic_lang_list.extend([topics_id2lang[tt] for tt in [t] * len(c)])
    ...
    if labels:
        return pd.DataFrame({'topic_id':topic_id_list, 'content_id':contents_id_list, 'topic_title':topic_title_list, 'content_title':contents_title_list, 'language':topic_lang_list, 'label':labels, 'fold':folds})
    else:
        return pd.DataFrame({'topic_id':topic_id_list, 'content_id':contents_id_list, 'topic_title':topic_title_list, 'content_title':contents_title_list,'language':topic_lang_list, 'fold':folds})
...
class Stage2model_deberta(pl.LightningModule):
...
def inference_stage2_pytorch(CFG, df, model_name, model_save_path):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = Stage2model(model_name) 
    state = torch.load(model_save_path, map_location = torch.device('cpu'))
    ...
    del model, tokenizer, dataset, dataloader
    gc.collect()
    return preds

def inference_stage2_pytorch_deberta(CFG, df, model_name, model_save_path):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = Stage2model_deberta(model_name) 
    state = torch.load(model_save_path, map_location = torch.device('cpu'))
    model.load_state_dict(state['state_dict'])
    del state
    dataset = MyDataset(tokenizer, df, labels=False)
...
def inference_pipline(CFG, topics_df, content_df, correlations_df, sample_submission, prev_df=None):
    ...
    if prev_df is not None:
        df = prev_df
    df = df.dropna(subset=['topic_title'])
    df = df.dropna(subset=['content_title'])
    df = df.reset_index(drop=True)
    if (prev_df is None) or ('predictions_proba_svm' not in prev_df.columns): 
        preds = inference_svm(CFG, df, topics_df, content_df, topics_embedding_cudf, contents_embedding_cudf)
        df['predictions_proba_svm'] = preds
    if (prev_df is None) or ('predictions_proba_xgb' not in prev_df.columns): 
        preds = inference_xgb(CFG, df, topics_df, content_df, topics_embedding_cudf, contents_embedding_cudf)
        df['predictions_proba_xgb'] = preds
    if (prev_df is None) or ('predictions_proba_roberta_large' not in prev_df.columns): 
        preds = inference_stage2_pytorch(CFG, df, CFG.STAGE2_MODEL_NAME_2, CFG.STAGE2_PRETRAINED_PATH_2)
        df['predictions_proba_roberta_large'] = preds
    if (prev_df is None) or ('predictions_proba_roberta_base' not in prev_df.columns): 
        preds = inference_stage2_pytorch(CFG, df, CFG.STAGE2_MODEL_NAME_1, CFG.STAGE2_PRETRAINED_PATH_1)
        df['predictions_proba_roberta_base'] = preds
    if (prev_df is None) or ('predictions_proba_deberta_large' not in prev_df.columns): 
        en_df = df[df['language']=='en']
        preds = inference_stage2_pytorch_deberta(CFG, en_df, CFG.STAGE2_MODEL_NAME_3, CFG.STAGE2_PRETRAINED_PATH_3)
        df['predictions_proba_deberta_large'] = np.nan
        df.loc[en_df.index, 'predictions_proba_deberta_large'] = preds
    return df

def main():
    CFG_LIST = [CFG1, CFG2, CFG3]
    result_df_list = []
    en_threhold_list = [0.80, 0.88, 0.58]
    no_en_threhold_list= [0.947, 0.674, 0.640]
    for model_idx, CFG in enumerate(CFG_LIST):
        LOGGER.info(f"stage1model:{CFG.STAGE1_MODEL_NAME}")
        ...
        if CFG_COMMON.TEST_PRED:
            ...
            result = ensemble_postprocess_2(df, 
                threshold=en_threhold_list[model_idx], weight=np.array(weight_list), top_n = 1)
        else:
            ...
            try:
                ...
                prev_df=None
            df = inference_pipline(CFG, topics_df, content_df, correlations_df, sample_submission, prev_df)  
            df.to_csv(df_path, index=False)
            target_df = correlations_df[correlations_df.topic_id.isin(df.topic_id)].reset_index(drop=True)
            no_en_df = df[df['language']!='en']
            no_en_result = ensemble_postprocess_2(no_en_df, columns = ['predictions_proba_svm','predictions_proba_roberta_base', 'predictions_proba_xgb'], 
                threshold=no_en_threhold_list[model_idx], weight=np.array([0.458, 0.530, 0.249]), top_n = 1)
            en_df = df[df['language']=='en']
            en_result = ensemble_postprocess_2(en_df, columns = ['predictions_proba_svm','predictions_proba_roberta_base', 'predictions_proba_xgb', 'predictions_proba_deberta_large'], 
                threshold=en_threhold_list[model_idx], weight=np.array([0.314, 0.269, 0.508, 0.302]), top_n = 1)
            result = pd.concat([no_en_result, en_result], axis=0).reset_index(drop=True)
            score = calc_f2_score_mean(target_df.sort_values('topic_id').reset_index(drop=True), result.sort_values('topic_id').reset_index(drop=True))
            LOGGER.info(f"ensemble_score:{score:.3f}")
